...
y(1:50) = 0.005065;
hold;
plot(t, get_event_signal(y, t, [HE]));
plot(t, get_event_signal(y, t, [CR]), 'red');
plot(t, get_event_signal(y, t, [SW]), 'green');
legend('HE', 'CR', 'SW')
%plot(t, get_event_signal(y, t, [HE]))
hold;
window_size = 42; % Answer to the Ultimate Question of Life, the Universe, and Everything
rows = size(y) ./ window_size;
rows = floor(rows(2));
avgs = mean(reshape(y, window_size, rows)); 
%plot(avgs)